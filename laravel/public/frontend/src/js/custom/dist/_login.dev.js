...
$(function () {
  ...
  $('#frmLogin').submit(function (evt) {
    ...
    if ($('#passwordField').is(":visible") == false && $('#login-password').val() != '') $('#login-password').val('');
    ...
    if ($("#loginIdentityNumber").val() == '') return;
    ...
    if ($(this).valid()) {
      $("#PolicyHolderUniqueId").prop("disabled", true); //if ($('#passwordField').is(":visible") == true)
      //    $('#login-password').val('');
...
(function ($) {
  ...
  $.fn.resetValidation = function () {
    ...
    $form.find("[data-valmsg-summary=true]").removeClass("validation-summary-errors").addClass("validation-summary-valid").find("ul").empty(); //reset unobtrusive field level, if it exists

    $form.find("[data-valmsg-replace]").removeClass("field-validation-error").addClass("field-validation-valid").empty();
    return $form;
  };
})(jQuery); //focus on first element when Login modal open
...
//# sourceMappingURL=_login.dev.js.map
